...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Convert a Hugging Face model to Core ML."
    )
    parser.add_argument(
        "--model",
        type=str,
    ...
    try:
        ...
        model = AutoModelForCausalLM.from_pretrained(
            downloaded_hf_model_dir, torch_dtype=torch.float16
        )
        model.eval()
        ...
        dummy_input_ids = torch.randint(
            0, tokenizer.vocab_size, (1, 10), dtype=torch.long
        )
        ...
        coreml_model = ct.convert(
            ...
            inputs=[
                ct.TensorType(
                    name="input_ids",
                    shape=(1, ct.RangeDim(upper_bound=max_seq_length)),
                    dtype=np.int32
                ),
                ct.TensorType(
                    name="attention_mask",
